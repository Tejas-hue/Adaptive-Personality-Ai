...
scaler = joblib.load("scaler_pandora.pkl")
labels = ['Agreeableness', 'Openness', 'Conscientiousness', 'Extraversion', 'Neuroticism']
print("Pandora model loaded successfully.")

def predict_pandora_scores(text):
    """Predicts personality scores for a given text."""
...
gr.Interface(
    fn=predict_pandora_scores, 
    inputs=gr.Textbox(lines=5, label="Input Text"), 
    outputs="json",
    title="Pandora Personality Trait Assessor",
    description="An API for the fine-tuned RoBERTa-large model that predicts Big Five personality scores from text."
).launch()
